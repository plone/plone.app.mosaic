...
class LayoutWidget(BaseWidget, TextAreaWidget):
    ...
    def get_options(self):
        ...
        html_tiles = [t for tg in result["tiles"] for t in tg["tiles"] if t["name"] == "plone.app.standardtiles.html"]

        if html_tiles:
            toolbar_actions = [a[8:] for a in html_tiles[0]["available_actions"] if a.startswith("toolbar-")]
            if toolbar_actions:
