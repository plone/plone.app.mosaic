...
$.fn.mosaicOverlay = function () {
    ...
    return this.each(function () {
        ...
        try {
            $(".pat-textareamimetypeselector").change();
        } catch (e) {
            // ignore.
        }
        ...
        // Add lightbox
        $(document.body, $.mosaic.document).prepend(
...
$.mosaic.overlay.open = function (mode, tile_config) {
    ...
    if (mode === "all" && $.mosaic.overlay_hide_fields) {
        ...
        // Hide layout field
        form.find($.mosaic.options.customContentLayout_selector).addClass(
            "mosaic-hidden"
        );
        ...
        for (x = 0; x < tile_group.tiles.length; x += 1) {
            ...
            if (
                $.mosaic.options.panels.find(".mosaic-" + field_tile.name + "-tile")
                    .length !== 0
            ) {
                $($.mosaic.document.getElementById(field_tile.id)).addClass(
                    "mosaic-hidden"
                );
            }
        }
