...
Tile.prototype.initialize = function () {
    ...
    this.$el.prepend(
        $($.mosaic.document.createElement("div"))
            ...
            .append(
                $($.mosaic.document.createElement("div")).addClass(
    ...
    // Add label
    if (tile_config) {
        var side_tools = $($.mosaic.document.createElement("div")).addClass(
...
Tile.prototype.initializeButtons = function () {
    ...
    if (buttons.length > 0) {
        var $btns = $($.mosaic.document.createElement("div")).addClass(
    ...
    if (tile_config.tile_type === "app") {
        ...
        $.mosaic.overlay.app.on("after-render", function (event) {
            ...
            if ($.mosaic.hasContentLayout) {
                ...
                $("form", $.mosaic.overlay.app.$modal).append(
...
Tile.prototype.makeMovable = function () {
    ...
    if (
        ...
        $.mosaic.options.canChangeLayout &&
...
Tile.prototype.saveForm = function () {
    ...
    if (
        ...
        switch (tile_config.widget) {
            ...
            case "z3c.form.browser.textlines.TextLinesFieldWidget":
                value = "";
                $(".mosaic-panel .mosaic-" + tiletype + "-tile", $.mosaic.document)
            ...
            case "plone.app.widgets.dx.RichTextWidget":
                ...
                var content = $(".mosaic-" + tiletype + "-tile", $.mosaic.document)
                    .find(".mosaic-tile-content")
                    .html();
...
Tile.prototype.setupWysiwyg = function () {
    ...
    while ($("#mosaic-rich-text-init-" + random_id, $.mosaic.document).length > 0) {
    ...
    pattern = new TinyMCE(
        $content,
        $.extend(true, {}, $.mosaic.options.tinymce, {
            inline: false,
            tiny: {
                body_id: id,
                selector: "#" + id,
