...
Mosaic uses a row-and-column grid system. You can configure the maximum number of columns available in a specific panel using the ``data-max-columns`` attribute.

``data-max-columns``
    Specifies the grid width (typically 4 or 12). This attribute must be placed on the element defining the ``content`` panel.
